    ...
    return {
        ...
        get filteredMembers() {
            return this.members.filter(m => 
                ...
                (m.id && m.id.toLowerCase().includes(this.search.toLowerCase())) ||
                (m.licensePlate && m.licensePlate.toLowerCase().includes(this.search.toLowerCase())) ||
                (m.vehicleModel && m.vehicleModel.toLowerCase().includes(this.search.toLowerCase()))
            );
        },

        get stats() {
            return [
                { label: "Total Members", value: this.members.length, icon: "users", bgClass: Alpine.store('app').isDark ? "bg-blue-500/20" : "bg-blue-50", iconClass: Alpine.store('app').isDark ? "text-blue-400" : "text-blue-600" },
                { label: "Active", value: this.members.filter(m => m.status === 'Active').length, icon: "user-check", bgClass: Alpine.store('app').isDark ? "bg-green-500/20" : "bg-green-50", iconClass: Alpine.store('app').isDark ? "text-green-400" : "text-green-600" },
                { label: "Inactive", value: this.members.filter(m => m.status === 'Inactive').length, icon: "user-x", bgClass: Alpine.store('app').isDark ? "bg-gray-500/20" : "bg-gray-100", iconClass: Alpine.store('app').isDark ? "text-gray-400" : "text-gray-500" },
                { label: "Suspended", value: this.members.filter(m => m.status === 'Suspended').length, icon: "user-x", bgClass: Alpine.store('app').isDark ? "bg-red-500/20" : "bg-red-50", iconClass: Alpine.store('app').isDark ? "text-red-400" : "text-red-500" },
            ];
        },
        ...
        getPlanBadgeClass(plan) {
            ...
            return map[plan] || "bg-gray-500/20 text-gray-400 border-gray-500/30";
        },
